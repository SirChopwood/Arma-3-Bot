...
async def Main(self, message, command, arguments):
    if arguments == "" or arguments is None or arguments == command:
        await message.channel.send(content="", embed=embedtemplates.help("Changes a Rank's Format."))
        return
    ...
    await message.channel.send(content="", embed=embedtemplates.success("Rank Edited", str("Rank Format changed to ``" + name.content + "``.")))
    for m in messages:
        await m.delete()
