...
async def Main(self, message, command, arguments):
    if arguments == "" or arguments is None or arguments == command:
        await message.channel.send(content="", embed=embedtemplates.help("Changes a Rank's Long Name."))
        return
    ...
    messages.append(
        await message.channel.send(content="", embed=embedtemplates.question("What is the new Long Name of the Rank?",
                                                                             message.author.display_name)))
    name = await self.await_response(message.author)
    messages.append(name)
    rank["Long"] = name.content

    self.database.set_ranks(message.guild.id, ranks)
    await message.channel.send(content="", embed=embedtemplates.success("Rank Edited", str("Rank Long Name changed to ``" + name.content + "``.")))
    for m in messages:
        await m.delete()
