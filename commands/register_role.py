...
async def Main(self, message, command, arguments):
    if arguments == 0 or arguments == "" or arguments == None or arguments == command:
        await message.channel.send(content="", embed=embedtemplates.help(
            "Changes the Discord Role that you get when Registering."))
        return
    ...
    try:
        settings["RegisterRole"] = int(message.role_mentions[0].id)
    ...
    await message.channel.send(content="", embed=embedtemplates.success("Settings Edited", str("Register RoleID changed to ``" + str(message.role_mentions[0].id) + "``.")))
